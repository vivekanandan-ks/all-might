            ...
            content=ft.Stack(
                [
                    ft.IconButton(
                        ...
                        on_click=lambda e: toggle_filter_menu(True),
                    ),
                    filter_badge_container,
                ]
            )
...
def get_lists_view(
    ...
    if selected_list_name or is_viewing_favourites:
        ...
        tooltip_cmd = (
            _build_shell_command_for_items(items_for_tooltip, with_wrapper=True)
            if items_for_tooltip
            else ""
        )
        clean_cmd = (
            _build_shell_command_for_items(items_for_tooltip, with_wrapper=False)
            if items_for_tooltip
            else ""
        )
        ...
        header_actions = [
            ...
            ft.Container(
                ...
                content=ft.Row(
                    ...
                    controls=[
                        ...
                        ft.Text(
                            btn_text, weight=ft.FontWeight.BOLD, color=ft.Colors.WHITE
                        ),
                    ],
                ),
            ...
            ft.IconButton(
                ft.Icons.CONTENT_COPY, on_click=copy_list_command, tooltip=clean_cmd
            ),
        ]
        if refresh_callback and state.show_refresh_button:
                            ...
                            controls=[
                                ft.Row(
                                    [
                                        ft.IconButton(
                                            ft.Icons.ARROW_BACK,
                                            on_click=go_back_to_lists_index,
                                        ),
                                        ft.Text(
                                            title, size=24, weight=ft.FontWeight.BOLD
                                        ),
                                    ]
                                ),
...
def create_stacked_card(content, base_color, width=None, height=None, expand=1):
    ...
    return ft.Container(
        ...
        height=height,
        content=ft.Stack(controls=[layer1, layer2, content]),
    )
...
class SongCard(GlassContainer):
    ...
    def initialize_state(self):
        ...
        if state.song_use_mastodon:
            ...
            if state.default_song_cache:
                self.title_text = state.default_song_cache.get(
    ...
    def fetch_mastodon_meta(self):
        ...
        fetched = get_mastodon_quote(
            state.song_mastodon_account, state.song_mastodon_tag
        )
        if fetched:
            # Parse Song Link
            ...
            fetched["fetched_at"] = datetime.datetime.now().strftime(
                "%Y-%m-%d %H:%M:%S"
            )
    ...
    def update_card_content(self):
        ...
        self.content = ft.Column(
            [
                ...
                ft.Container(
                    content=ft.Row(
                        [thumbnail_control, text_content],
                        spacing=15,
    ...
    def handle_click(self, e):
                ...
                ft.Row(
                    [
                        ft.ElevatedButton(
                            ...
                            on_click=open_song,
                            width=220,
                            tooltip=self.target_url,
                        ),
                        ft.IconButton(
                            ft.Icons.COPY,
                            tooltip="Copy Song Link",
                            on_click=lambda e: copy_text(e, self.target_url),
                        ),
                    ],
                    alignment=ft.MainAxisAlignment.CENTER,
                ),
                ...
                ft.Row(
                    [
                        ft.ElevatedButton(
                            "Open Mastodon Post",
                            icon=ft.Icons.FORUM,
                            on_click=open_mastodon,
                            width=220,
                            disabled=not self.mastodon_url,
                            tooltip=self.mastodon_url,
                        ),
                        ft.IconButton(
                            ft.Icons.COPY,
                            tooltip="Copy Mastodon Link",
                            on_click=lambda e: copy_text(e, self.mastodon_url),
                            disabled=not self.mastodon_url,
                        ),
                    ],
                    alignment=ft.MainAxisAlignment.CENTER,
        ...
        actions = [
            ft.IconButton(
                ft.Icons.REFRESH, tooltip="Refresh Metadata", on_click=refresh_meta
            ),
            ft.TextButton("Close", on_click=lambda e: close_func[0]()),
        ]

        if controls_mod.show_glass_dialog:
            close_func[0] = controls_mod.show_glass_dialog(
                "Link Options", dlg_content, actions
            )
...
def get_home_view():
    ...
    def create_dynamic_card_click_handler(link, refresh_callback=None):
        def handler(e):
                    ...
                    ft.Row(
                        [
                            ...
                            ft.IconButton(
                                ft.Icons.COPY,
                                tooltip="Copy Link",
                                on_click=lambda e: copy_link(e, link),
                            ),
                        ],
                        alignment=ft.MainAxisAlignment.CENTER,
                    ),
            ...
            if refresh_callback:
                actions.append(
                    ft.IconButton(
                        ft.Icons.REFRESH, tooltip="Refresh", on_click=refresh_action
                    )
                )

            actions.append(ft.TextButton("Close", on_click=lambda e: close_dialog[0]()))

            close_dialog[0] = controls_mod.show_glass_dialog(
                "Link Options", dlg_content, actions
            )
    ...
    if cfg["visible"]:
        ...
        if state.app_use_mastodon:
            if not state.app_mastodon_cache:
                fetched = get_mastodon_quote(
                    ...
                    server=state.app_mastodon_server,
                )
                if fetched:
            ...
            content=ft.Column(
                ...
                controls=[
                    ft.Row(
                        ...
                        alignment=get_alignment(cfg["align"]),
                    ),
                    ft.Row(
                        [
                            ft.Text(
        ...
        cards_row1.append(
            create_stacked_card(
                main_card, base_col, height=cfg["h"], width=cfg["w"], expand=2
            )
        )
    ...
    if cfg["visible"]:
        ...
        if state.tip_use_mastodon:
            if not state.tip_mastodon_cache:
                fetched = get_mastodon_quote(
                    ...
                    server=state.tip_mastodon_server,
                )
                if fetched:
        ...
        cards_row1.append(
            create_stacked_card(
                main_card, base_col, height=cfg["h"], width=cfg["w"], expand=1
            )
        )
    ...
    if cfg["visible"]:
        ...
        def fetch_fresh_quote(force_refresh=False):
            ...
            if state.use_mastodon_quote:
                fetched = get_mastodon_quote(
                    state.quote_mastodon_account,
                    state.quote_mastodon_tag,
                    server=state.quote_mastodon_server,
                )
                if fetched == {}:
                    ...
                    if link:
                        main_card.tooltip = "Click for options"
                        q_click_handler[0] = create_dynamic_card_click_handler(
                            link, refresh_quote_action
                        )
        ...
        def refresh_quote_action():
            ...
            threading.Thread(
                target=lambda: fetch_fresh_quote(force_refresh=True), daemon=True
            ).start()
        ...
        q_text_control = ft.Text(
            ...
            weight=ft.FontWeight.BOLD
            if state.quote_style_bold
            else ft.FontWeight.NORMAL,
            text_align=ft.TextAlign.LEFT
            if cfg["align"] == "left"
            else (
                ft.TextAlign.RIGHT if cfg["align"] == "right" else ft.TextAlign.CENTER
            ),
        )
            ...
            content=ft.Column(
                controls=[
                    ...
                    ft.Container(
                        expand=True,
                        content=ft.Column(
                            [q_text_control], alignment=ft.MainAxisAlignment.CENTER
                        ),
                    ),
                ]
            ),
    ...
    if state.carousel_use_mastodon and state.carousel_mastodon_cache:
        ...
        for i, item in enumerate(state.carousel_mastodon_cache):
            cached_items.append(
                {
                    "title": "Community Tip",
                    "desc": item.get("text", "")[:150] + "..."
                    if len(item.get("text", "")) > 150
                    else item.get("text", ""),
    ...
    def fetch_fresh_carousel_data():
        if (
            state.carousel_use_mastodon
            and state.carousel_mastodon_account
            and state.carousel_mastodon_tag
        ):
            try:
                feed = get_mastodon_feed(
                    state.carousel_mastodon_account,
                    state.carousel_mastodon_tag,
                    limit=5,
                    server=state.carousel_mastodon_server,
                )
                if feed:
        ...
        controls=[
            ft.Column(
                [
                    ...
                    ft.Text(
                        "Welcome to All Might",
                        size=state.get_size(1.15),
                        color="onSurfaceVariant",
                    ),
                ]
            ),
...
def get_settings_view(
    ...
    new_channel_input = ft.TextField(
        hint_text="nixos-23.11",
        expand=True,
        height=40,
        text_size=12,
        content_padding=10,
    ...
    preview_text_normal = ft.Text(
        "This is how your text looks.",
        size=state.get_font_size("body"),
        color="onSurface",
    )
    preview_text_small = ft.Text(
        "Small text example",
        size=state.get_font_size("small"),
        color="onSurfaceVariant",
    )
    preview_text_title = ft.Text(
        "Large Header", size=state.get_font_size("title"), weight=ft.FontWeight.BOLD
    )
    ...
    txt_global_font = ft.Text(
        get_label_text("Global Font Size", state.global_font_size, 14),
        weight=ft.FontWeight.BOLD,
    )
    txt_title_font = ft.Text(
        get_label_text("Title Font Size", state.title_font_size, 16)
    )
    txt_body_font = ft.Text(get_label_text("Body Font Size", state.body_font_size, 14))
    txt_small_font = ft.Text(
        get_label_text("Small/Tag Font Size", state.small_font_size, 12)
    )
    ...
    txt_nav_width = ft.Text(
        get_label_text("Total Length (Floating)", state.nav_bar_width, 500)
    )
    txt_nav_height = ft.Text(get_label_text("Nav Bar Height", state.nav_bar_height, 80))
    txt_nav_spacing = ft.Text(
        get_label_text("Icon Spacing (Manual)", state.nav_icon_spacing, 15)
    )
    ...
    txt_global_radius = ft.Text(
        get_label_text("Global Radius", state.global_radius, 33),
        weight=ft.FontWeight.BOLD,
    )
    ...
    txt_button_radius = ft.Text(
        get_label_text("Button Radius", state.button_radius, 10)
    )
    txt_search_radius = ft.Text(
        get_label_text("Search Bar Radius", state.search_radius, 15)
    )
    txt_selector_radius = ft.Text(
        get_label_text("Selector Radius", state.selector_radius, 15)
    )
    txt_footer_radius = ft.Text(
        get_label_text("Footer Section Radius", state.footer_radius, 15)
    )
    txt_chip_radius = ft.Text(
        get_label_text("Footer Chip Radius", state.chip_radius, 10)
    )
    ...
    def reset_with_confirmation(
        title, default_applier, undo_state_capturer, undo_restorer
    ):
        ...
        show_destructive_dialog(
            title, "Are you sure you want to reset settings to defaults?", on_confirm
        )

    def reset_radius_defaults(e):
        def capture():
            return {
                "global": state.global_radius,
                "nav": state.nav_radius,
                "sync_nav": state.sync_nav_radius,
                "card": state.card_radius,
                "sync_card": state.sync_card_radius,
                "btn": state.button_radius,
                "sync_btn": state.sync_button_radius,
                "search": state.search_radius,
                "sync_search": state.sync_search_radius,
                "sel": state.selector_radius,
                "sync_sel": state.sync_selector_radius,
                "foot": state.footer_radius,
                "sync_foot": state.sync_footer_radius,
                "chip": state.chip_radius,
                "sync_chip": state.sync_chip_radius,
            }

        def apply():
            state.global_radius = 33
            state.nav_radius = 33
            state.sync_nav_radius = True
            state.card_radius = 15
            state.sync_card_radius = True
            state.button_radius = 10
            state.sync_button_radius = True
            state.search_radius = 15
            state.sync_search_radius = True
            state.selector_radius = 15
            state.sync_selector_radius = True
            state.footer_radius = 15
            state.sync_footer_radius = True
            state.chip_radius = 10
            state.sync_chip_radius = True
        ...
        def restore(s):
            state.global_radius = s["global"]
            state.nav_radius = s["nav"]
            state.sync_nav_radius = s["sync_nav"]
            state.card_radius = s["card"]
            state.sync_card_radius = s["sync_card"]
            state.button_radius = s["btn"]
            state.sync_button_radius = s["sync_btn"]
            state.search_radius = s["search"]
            state.sync_search_radius = s["sync_search"]
            state.selector_radius = s["sel"]
            state.sync_selector_radius = s["sync_sel"]
            state.footer_radius = s["foot"]
            state.sync_footer_radius = s["sync_foot"]
            state.chip_radius = s["chip"]
            state.sync_chip_radius = s["sync_chip"]
    ...
    def reset_navbar_defaults(e):
        def capture():
            return {
                "float": state.floating_nav,
                "adapt": state.adaptive_nav,
                "glass": state.glass_nav,
                "w": state.nav_bar_width,
                "h": state.nav_bar_height,
                "space": state.nav_icon_spacing,
                "sync_space": state.sync_nav_spacing,
                "badge": state.nav_badge_size,
            }

        def apply():
            state.floating_nav = True
            state.adaptive_nav = True
            state.glass_nav = True
            state.nav_bar_width = 500
            state.nav_bar_height = 80
            state.nav_icon_spacing = 15
            state.sync_nav_spacing = True
            state.nav_badge_size = 20
        ...
        def restore(s):
            state.floating_nav = s["float"]
            state.adaptive_nav = s["adapt"]
            state.glass_nav = s["glass"]
            state.nav_bar_width = s["w"]
            state.nav_bar_height = s["h"]
            state.nav_icon_spacing = s["space"]
            state.sync_nav_spacing = s["sync_space"]
            state.nav_badge_size = s["badge"]

        reset_with_confirmation("Reset Navbar Defaults?", apply, capture, restore)

    def reset_timer_defaults(e):
        def capture():
            return {"confirm": state.confirm_timer, "undo": state.undo_timer}

        def apply():
            state.confirm_timer = 5
            state.undo_timer = 5

        def restore(s):
            state.confirm_timer = s["confirm"]
            state.undo_timer = s["undo"]
    ...
    def reset_font_defaults(e):
        def capture():
            return {
                "global": state.global_font_size,
                "title": state.title_font_size,
                "sync_title": state.sync_title_font,
                "body": state.body_font_size,
                "sync_body": state.sync_body_font,
                "small": state.small_font_size,
                "sync_small": state.sync_small_font,
                "nav": state.nav_font_size,
                "sync_nav": state.sync_nav_font,
            }

        def apply():
            state.global_font_size = 14
            state.title_font_size = 16
            state.sync_title_font = True
            state.body_font_size = 14
            state.sync_body_font = True
            state.small_font_size = 12
            state.sync_small_font = True
            state.nav_font_size = 12
            state.sync_nav_font = True
        ...
        def restore(s):
            state.global_font_size = s["global"]
            state.title_font_size = s["title"]
            state.sync_title_font = s["sync_title"]
            state.body_font_size = s["body"]
            state.sync_body_font = s["sync_body"]
            state.small_font_size = s["small"]
            state.sync_small_font = s["sync_small"]
            state.nav_font_size = s["nav"]
            state.sync_nav_font = s["sync_nav"]
    ...
    def create_card_config_tile(card_key, label):
        ...
        txt_width = ft.Text(
            get_label_text("Width (px) [0 = Auto]", cfg["w"], default["w"])
        )
        ...
        def update_card_color(e):
            ...
            for ctrl in color_row.controls:
                ...
                ctrl.border = (
                    ...
                    if is_sel
                    else ft.border.all(2, ft.Colors.TRANSPARENT)
                )
            color_row.update()

        switch_visible = ft.Switch(value=cfg["visible"], on_change=update_visible)
        slider_height = ft.Slider(
            min=100, max=400, value=cfg["h"], label="{value}", on_change=update_height
        )
        slider_width = ft.Slider(
            min=0, max=600, value=cfg["w"], label="{value}", on_change=update_width
        )
        seg_align = ft.SegmentedButton(
            ...
            segments=[
                ft.Segment(
                    value="left",
                    label=ft.Text("Left"),
                    icon=ft.Icon(ft.Icons.FORMAT_ALIGN_LEFT),
                ),
                ft.Segment(
                    value="center",
                    label=ft.Text("Center"),
                    icon=ft.Icon(ft.Icons.FORMAT_ALIGN_CENTER),
                ),
                ft.Segment(
                    value="right",
                    label=ft.Text("Right"),
                    icon=ft.Icon(ft.Icons.FORMAT_ALIGN_RIGHT),
                ),
            ],
        )
        ...
        def reset_card_defaults(e):
            ...
            def apply():
                ...
                txt_height.value = get_label_text(
                    "Height (px)", default["h"], default["h"]
                )

                slider_width.value = default["w"]
                txt_width.value = get_label_text(
                    "Width (px) [0 = Auto]", default["w"], default["w"]
                )
        ...
        tile_content = [
            ft.Row(
                [ft.Text("Show Card:", weight=ft.FontWeight.BOLD), switch_visible],
                alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
            ),
            ft.Divider(),
            txt_height,
            ...
            txt_width,
            slider_width,
            ft.Text("Content Alignment:"),
            seg_align,
            ft.Container(height=10),
            ft.Text(f"Card Color (Def: {default['color'].capitalize()})"),
            color_row,
        ]
        ...
        if card_key == "quote":
            ...
            tile_content.extend(
                [
                    ...
                    ft.Row(
                        [
                            ft.Text("Enable RSS Fetch:"),
                            ft.Switch(
                                value=state.use_mastodon_quote,
                                on_change=update_use_mastodon,
                            ),
                        ],
                        alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                    ),
                    ft.TextField(
                        label="Server (e.g. mstdn.social)",
                        value=state.quote_mastodon_server,
                        on_blur=update_mastodon_server,
                        text_size=12,
                    ),
                    ft.TextField(
                        label="Account (e.g. vivekanandanks)",
                        value=state.quote_mastodon_account,
                        on_blur=update_mastodon_account,
                        text_size=12,
                    ),
                    ft.TextField(
                        label="Tag (e.g. mha)",
                        value=state.quote_mastodon_tag,
                        on_blur=update_mastodon_tag,
                        text_size=12,
                    ),
                    ft.Container(height=10),
                    ...
                    ft.Row(
                        [
                            ft.Text("Italic Text:"),
                            ft.Switch(
                                value=state.quote_style_italic,
                                on_change=update_quote_italic,
                            ),
                        ],
                        alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                    ),
                    ft.Row(
                        [
                            ft.Text("Bold Text:"),
                            ft.Switch(
                                value=state.quote_style_bold,
                                on_change=update_quote_bold,
                            ),
                        ],
                        alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                    ),
                ...
                [
                    ...
                    ft.Row(
                        [
                            ft.Text("Enable RSS Fetch:"),
                            ft.Switch(
                                value=state.app_use_mastodon,
                                on_change=update_app_mastodon,
                            ),
                        ],
                        alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                    ),
                    ft.TextField(
                        label="Server",
                        value=state.app_mastodon_server,
                        on_blur=update_app_server,
                        text_size=12,
                    ),
                    ft.TextField(
                        label="Account",
                        value=state.app_mastodon_account,
                        on_blur=update_app_account,
                        text_size=12,
                    ),
                    ft.TextField(
                        label="Tag",
                        value=state.app_mastodon_tag,
                        on_blur=update_app_tag,
                        text_size=12,
                    ),
                ]
            )
                ...
                [
                    ...
                    ft.Row(
                        [
                            ft.Text("Enable RSS Fetch:"),
                            ft.Switch(
                                value=state.tip_use_mastodon,
                                on_change=update_tip_mastodon,
                            ),
                        ],
                        alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                    ),
                    ft.TextField(
                        label="Server",
                        value=state.tip_mastodon_server,
                        on_blur=update_tip_server,
                        text_size=12,
                    ),
                    ft.TextField(
                        label="Account",
                        value=state.tip_mastodon_account,
                        on_blur=update_tip_account,
                        text_size=12,
                    ),
                    ft.TextField(
                        label="Tag",
                        value=state.tip_mastodon_tag,
                        on_blur=update_tip_tag,
                        text_size=12,
                    ),
                ]
            )
                ...
                [
                    ...
                    ft.Row(
                        [
                            ft.Text("Enable RSS Fetch:"),
                            ft.Switch(
                                value=state.song_use_mastodon,
                                on_change=update_song_mastodon,
                            ),
                        ],
                        alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                    ),
                    ft.TextField(
                        label="Server",
                        value=state.song_mastodon_server,
                        on_blur=update_song_server,
                        text_size=12,
                    ),
                    ft.TextField(
                        label="Account",
                        value=state.song_mastodon_account,
                        on_blur=update_song_account,
                        text_size=12,
                    ),
                    ft.TextField(
                        label="Tag",
                        value=state.song_mastodon_tag,
                        on_blur=update_song_tag,
                        text_size=12,
                    ),
                ]
            )
                ...
                [
                    ...
                    ft.Row(
                        [
                            ft.Text("Enable RSS Fetch:"),
                            ft.Switch(
                                value=state.carousel_use_mastodon,
                                on_change=update_carousel_mastodon,
                            ),
                        ],
                        alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                    ),
                    ft.TextField(
                        label="Server",
                        value=state.carousel_mastodon_server,
                        on_blur=update_carousel_server,
                        text_size=12,
                    ),
                    ft.TextField(
                        label="Account",
                        value=state.carousel_mastodon_account,
                        on_blur=update_carousel_account,
                        text_size=12,
                    ),
                    ft.TextField(
                        label="Tag",
                        value=state.carousel_mastodon_tag,
                        on_blur=update_carousel_tag,
                        text_size=12,
                    ),
                ]
            )
    ...
    def get_settings_controls(category):
        ...
        if category == "history":
            ...
            carousel_timer_input = ft.TextField(
                ...
                filled=True,
                bgcolor=ft.Colors.with_opacity(0.1, "onSurface"),
                on_submit=update_carousel_timer,
                on_blur=update_carousel_timer,
            )
            controls_list = [
                ft.Text("Home Page Configuration", size=24, weight=ft.FontWeight.BOLD),
                ft.Divider(),
                ft.Row(
                    [ft.Text("Carousel Timer (s):"), carousel_timer_input],
                    alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                ),
                ft.Row(
                    [
                        ft.Text("Countdown Effect Animation:"),
                        ft.Switch(
                            value=state.carousel_glass, on_change=update_carousel_glass
                        ),
                    ],
                    alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                ),
            ...
            controls_list = [
                ...
                make_settings_tile(
                    "Theme",
                    [
                        ft.Text("Background Image:", weight=ft.FontWeight.BOLD),
                        bg_image_row,
                        ft.Container(height=10),
                        ft.Text("Background Brightness:", weight=ft.FontWeight.BOLD),
                        ft.Row(
                            [bg_opacity_slider, txt_bg_opacity],
                            alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                        ),
                        ft.Container(height=5),
                        ft.Text("Background Blur:", weight=ft.FontWeight.BOLD),
                        ft.Row(
                            [bg_blur_slider, txt_bg_blur],
                            alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                        ),
                        ft.Container(height=5),
                        ft.Row(
                            [
                                ft.Text(
                                    "Rotate Background:", weight=ft.FontWeight.BOLD
                                ),
                                ft.Switch(
                                    value=state.bg_rotation, on_change=update_bg_rot
                                ),
                            ],
                            alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                        ),
                    ...
                    [
                        ...
                        ft.Row(
                            [
                                txt_nav_radius,
                                ft.Switch(
                                    value=state.sync_nav_radius,
                                    label="Global",
                                    on_change=update_sync_nav_radius,
                                ),
                            ],
                            alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                        ),
                        ...
                        ft.Row(
                            [
                                txt_card_radius,
                                ft.Switch(
                                    value=state.sync_card_radius,
                                    label="Global",
                                    on_change=update_sync_card_radius,
                                ),
                            ],
                            alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                        ),
                        ...
                        ft.Row(
                            [
                                txt_button_radius,
                                ft.Switch(
                                    value=state.sync_button_radius,
                                    label="Global",
                                    on_change=update_sync_button_radius,
                                ),
                            ],
                            alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                        ),
                        ...
                        ft.Row(
                            [
                                txt_search_radius,
                                ft.Switch(
                                    value=state.sync_search_radius,
                                    label="Global",
                                    on_change=update_sync_search_radius,
                                ),
                            ],
                            alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                        ),
                        ...
                        ft.Row(
                            [
                                txt_selector_radius,
                                ft.Switch(
                                    value=state.sync_selector_radius,
                                    label="Global",
                                    on_change=update_sync_selector_radius,
                                ),
                            ],
                            alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                        ),
                        ...
                        ft.Row(
                            [
                                txt_footer_radius,
                                ft.Switch(
                                    value=state.sync_footer_radius,
                                    label="Global",
                                    on_change=update_sync_footer_radius,
                                ),
                            ],
                            alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                        ),
                        ...
                        ft.Row(
                            [
                                txt_chip_radius,
                                ft.Switch(
                                    value=state.sync_chip_radius,
                                    label="Global",
                                    on_change=update_sync_chip_radius,
                                ),
                            ],
                            alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                        ),
                    ...
                    [
                        ft.Row(
                            [
                                ...
                                ft.Switch(
                                    value=state.floating_nav,
                                    on_change=update_floating_nav,
                                ),
                            ],
                            alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                        ),
                        ft.Row(
                            [
                                ...
                                ft.Switch(
                                    value=state.adaptive_nav,
                                    on_change=update_adaptive_nav,
                                ),
                            ],
                            alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                        ),
                        ...
                        ft.Row(
                            [
                                ...
                                ft.Switch(
                                    value=state.sync_nav_spacing,
                                    on_change=update_sync_nav_spacing,
                                ),
                            ],
                            alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                        ),
                        ...
                        ft.Row(
                            [
                                ...
                                ft.Switch(
                                    value=state.glass_nav, on_change=update_glass_nav
                                ),
                            ],
                            alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                        ),
                        ft.Container(height=10),
                        ft.Row(
                            [ft.Text("Nav Badge Size:"), badge_size_input],
                            alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                        ),
                    ],
                ...
                make_settings_tile(
                    ...
                    [
                        ft.Row(
                            [ft.Text("Confirm Dialog (s):"), confirm_timer_input],
                            alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                        ),
                        ft.Container(height=10),
                        ft.Row(
                            [ft.Text("Undo Toast (s):"), undo_timer_input],
                            alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                        ),
                    ],
                    ...
                    [
                        ...
                        ft.Row(
                            [
                                txt_title_font,
                                ft.Switch(
                                    value=state.sync_title_font,
                                    label="Global",
                                    on_change=update_sync_title,
                                ),
                            ],
                            alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                        ),
                        ...
                        ft.Row(
                            [
                                txt_body_font,
                                ft.Switch(
                                    value=state.sync_body_font,
                                    label="Global",
                                    on_change=update_sync_body,
                                ),
                            ],
                            alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                        ),
                        ...
                        ft.Row(
                            [
                                txt_small_font,
                                ft.Switch(
                                    value=state.sync_small_font,
                                    label="Global",
                                    on_change=update_sync_small,
                                ),
                            ],
                            alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                        ),
                        ...
                        ft.Row(
                            [
                                txt_nav_font,
                                ft.Switch(
                                    value=state.sync_nav_font,
                                    label="Global",
                                    on_change=update_sync_nav_font,
                                ),
                            ],
                            alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                        ),
                ...
                make_settings_tile(
                    ...
                    [
                        ft.Row(
                            [
                                ft.Text("Fetch Icons for Search Results:"),
                                ft.Switch(
                                    value=state.fetch_icons,
                                    on_change=update_fetch_icons,
                                ),
                            ],
                            alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                        ),
            ...
            search_limit_input = ft.TextField(
                value=str(state.search_limit),
                width=100,
                height=40,
                text_size=12,
                content_padding=10,
                filled=True,
                bgcolor=ft.Colors.with_opacity(0.1, "onSurface"),
                on_submit=update_search_limit,
                on_blur=update_search_limit,
            )
            ...
            def add_custom_channel(e):
                ...
                if state.add_channel(ch_name):
                    ...
                    show_toast("Channel already exists")
            ...
            def remove_channel_dialog(channel_name):
                ...
                show_destructive_dialog(
                    "Remove Channel?",
                    f"Are you sure you want to remove '{channel_name}'?",
                    do_remove,
                )
            ...
            def toggle_channel_active(channel_name, is_active):
                ...
                refresh_dropdown_options()
                show_toast(
                    f"{channel_name} is now {'active' if is_active else 'inactive'}"
                )
            ...
            def update_channel_list():
                ...
                for channel in state.available_channels:
                    ...
                    chip = ft.Chip(
                        ...
                        selected=is_active,
                        on_select=lambda e, ch=channel: toggle_channel_active(
                            ch, e.control.selected
                        ),
                        on_delete=(lambda e, ch=channel: remove_channel_dialog(ch))
            ...
            suggestion_text = (
                f"Suggested: nixos-unstable, nixos-{prev_ver}, nixos-{next_ver}"
            )
            ...
            controls_list = [
                ...
                make_settings_tile(
                    ...
                    [
                        ...
                        ft.Row(
                            [ft.Text("Max results:", size=12), search_limit_input],
                            alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                        ),
                        ...
                        ft.Dropdown(
                            options=[
                                ft.dropdown.Option(c) for c in state.available_channels
                            ],
                            value=state.default_channel,
                            on_change=update_default_channel,
            ...
            cmd_preview_single = ft.Text(
                f"Preview: {state.shell_single_prefix} nix run nixpkgs/unstable#cowsay {state.shell_single_suffix}",
                size=12,
                color="onSurfaceVariant",
                font_family="monospace",
            )
            cmd_preview_cart = ft.Text(
                f"Preview: {state.shell_cart_prefix} nix shell nixpkgs/unstable#vim nixpkgs/unstable#git {state.shell_cart_suffix}",
                size=12,
                color="onSurfaceVariant",
                font_family="monospace",
            )
            ...
            controls_list = [
                ...
                make_settings_tile(
                    ...
                    [
                        ft.Text(
                            "Run without installing cmd config",
                            weight=ft.FontWeight.BOLD,
                        ),
                        ft.Container(height=5),
                        ft.Text("Prefix", weight=ft.FontWeight.BOLD),
                        ft.TextField(
                            value=state.shell_single_prefix,
                            hint_text="nix run",
                            text_size=12,
                            filled=True,
                            bgcolor=ft.Colors.with_opacity(0.1, "onSurface"),
                            on_change=update_shell_single_prefix,
                        ),
                        ft.Text("Suffix", weight=ft.FontWeight.BOLD),
                        ft.TextField(
                            value=state.shell_single_suffix,
                            hint_text="",
                            text_size=12,
                            filled=True,
                            bgcolor=ft.Colors.with_opacity(0.1, "onSurface"),
                            on_change=update_shell_single_suffix,
                        ),
                        cmd_preview_single,
                    ],
                ),
                ...
                make_settings_tile(
                    ...
                    [
                        ft.Text(
                            "Cart/List try in shell cmd config",
                            weight=ft.FontWeight.BOLD,
                        ),
                        ft.Container(height=5),
                        ft.Text("Prefix", weight=ft.FontWeight.BOLD),
                        ft.TextField(
                            value=state.shell_cart_prefix,
                            hint_text="nix shell",
                            text_size=12,
                            filled=True,
                            bgcolor=ft.Colors.with_opacity(0.1, "onSurface"),
                            on_change=update_shell_cart_prefix,
                        ),
                        ft.Text("Suffix", weight=ft.FontWeight.BOLD),
                        ft.TextField(
                            value=state.shell_cart_suffix,
                            hint_text="",
                            text_size=12,
                            filled=True,
                            bgcolor=ft.Colors.with_opacity(0.1, "onSurface"),
                            on_change=update_shell_cart_suffix,
                        ),
                        cmd_preview_cart,
                    ],
                ),
            ...
            interval_input = ft.TextField(
                value=str(state.auto_refresh_interval),
                width=100,
                height=40,
                text_size=12,
                content_padding=10,
                filled=True,
                bgcolor=ft.Colors.with_opacity(0.1, "onSurface"),
                on_submit=update_refresh_interval,
                on_blur=update_refresh_interval,
            )

            controls_list = [
                ft.Text("Installed Apps", size=24, weight=ft.FontWeight.BOLD),
                ft.Divider(),
                make_settings_tile(
                    "Refresh Settings",
                    [
                        ...
                        ft.Text(
                            "Automatically check installed packages status in background.",
                            size=12,
                            color="onSurfaceVariant",
                        ),
                        ft.Container(height=10),
                        ft.Row(
                            [
                                ft.Text("Enable Auto Refresh:"),
                                ft.Switch(
                                    value=state.auto_refresh_ui,
                        ...
                        ft.Row(
                            [ft.Text("Interval (seconds):"), interval_input],
                            alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                        ),
                    ],
                ),
            ...
            controls_list = [
                ft.Text("Debug Settings", size=24, weight=ft.FontWeight.BOLD),
                ft.Divider(),
                make_settings_tile(
                    "UI Options",
                    [
                        ft.Row(
                            [
                                ft.Text("Show Refresh Button:"),
                                ft.Switch(
                                    value=state.show_refresh_button,
                                    on_change=update_show_refresh,
                                ),
                            ],
                            alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                        )
            ...
            controls_list = [
                ft.Text("Experimental Settings", size=24, weight=ft.FontWeight.BOLD),
                ft.Divider(),
                ft.Text(
                    "No experimental settings currently.", color="onSurfaceVariant"
                ),
            ]
    ...
    settings_nav_rail = ft.NavigationRail(
        ...
        destinations=[
            ...
            ft.NavigationRailDestination(
                icon=ft.Icons.TV_OUTLINED, selected_icon=ft.Icons.TV, label="Channels"
            ),
            ft.NavigationRailDestination(
...
def make_settings_tile(title, controls, reset_func=None):
    ...
    if reset_func:
        ...
        expansion_controls.append(
            ft.Row([reset_btn], alignment=ft.MainAxisAlignment.END)
        )
...
def _launch_shell_dialog(display_cmd, title, page):
    ...
    if controls_mod.show_glass_dialog:
        close_func[0] = controls_mod.show_glass_dialog(
            f"Launching {title}", content, actions
        )
    ...
    try:
        ...
        try:
            ...
            if proc.returncode != 0:
                err_msg = (
                    errs.decode("utf-8", errors="replace") if errs else "Unknown error"
                )
                output_text.value = (
