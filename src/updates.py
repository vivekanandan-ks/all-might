...
def extract_attr_set(attr_path):
    if not attr_path: return "installed"
...
def get_installed_packages():
    try:
        ...
        for key, info in elements.items():
            ...
            if is_tracked:
                tracked_data = state.tracked_installs.get(state._get_track_key(name, channel))
            ...
            if not is_tracked:
                ...
                if tracked_channel:
                    ...
                    tracked_data = state.tracked_installs.get(state._get_track_key(name, tracked_channel))
            ...
            if is_tracked and tracked_data:
                ...
                clean_desc = original_url.replace("flake:", "") if original_url else "Unknown source"
...
def get_installed_view(page, on_cart_change_callback, show_toast_callback, refresh_callback=None):
    ...
    def update_view():
        ...
        filter_row.controls[0].label.value = f"Installed by All-Might ({count_all_might})"
    ...
    filter_row = ft.Row(
        controls=[
            ft.Chip(label=ft.Text("Installed by All-Might"), data="all-might", selected=True, on_select=on_filter_change),
            ft.Chip(label=ft.Text("Externally Installed"), data="external", selected=False, on_select=on_filter_change),
            ft.Chip(label=ft.Text("All"), data="all", selected=False, on_select=on_filter_change),
        ]
    )
